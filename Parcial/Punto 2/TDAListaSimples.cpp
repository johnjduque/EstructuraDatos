...
void mostrarFechasMayores(TDAnodo *duqLista){
	...
	if(esListaVacia(duqLista)){
		cout<<endl<<"no hay elementos para mostrar";
	}
	else{
		cout<<"Ingrese día a evaluar";
		cin>>john.dia;
		cout<<"Ingrese mes (como número) a evaluar";
		cin>>john.mes;
		cout<<"Ingrese año a evaluar";
		cin>>john.year;
		
		aux = duqLista;
		int convertirFechaLista = (aux->info.dia) + ((aux->info.mes)*2) + (aux->info.year);
		int convertirFechaEvuluar = (john.dia) + ((jonh.mes)*2) + (john.year);
		
		while((aux != NULL) && (convertirFechaEvuluar < convertirFechaLista)){
			cout<<" La dir: "<<aux;
			mostrarLista(aux);
			aux2 = aux;
			aux = aux->sig;
